...
import { apiDeleteWritingTest, apiFetchWritingTests } from '@/lib/api';
import type { WritingTest } from '@/lib/types';
import { useAuth } from '@/store/auth';
import { useFocusEffect } from '@react-navigation/native';
import { useRouter } from 'expo-router';
import { ArrowLeft, Check, ChevronRight, FileText, Plus, Trash2, X } from 'lucide-react-native';
...
export default function WritingTestManagementScreen() {
  const { user } = useAuth();
  ...
  const handleBulkDelete = () => {
    ...
    alert(
      `Delete ${count} Writing Test${count > 1 ? 's' : ''}?`,
      `This will permanently delete ${count} writing test${count > 1 ? 's' : ''} and all associated tasks and responses.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              await Promise.all(ids.map((id) => apiDeleteWritingTest(id)));
              setTests((prev) => prev.filter((t) => !selected.has(t.id)));
              toast.success('Deleted', `${count} writing test${count > 1 ? 's' : ''} deleted`);
              exitSelectMode();
            } catch (e: any) {
              toast.error('Error', e.message);
            } finally {
    ...
    <SafeAreaView style={styles.safeArea}>
      {selectMode ? (
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => router.push('/writing/create' as any)} activeOpacity={0.7}>
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  if (selectMode) {
                    ...
                    router.push({ pathname: '/writing/[id]', params: { id: String(item.id) } } as any);
                  }
                }}
                onLongPress={() => {
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.testSub}>{item.tasks?.length ?? 0} tasks</Text>
                </View>
          ...
          ListEmptyComponent={
            <View style={styles.centered}>
              ...
              <Text style={styles.emptyText}>
                {activeTab === 'all' ? 'No writing tests yet' : `No ${activeTab.toUpperCase()} writing tests found`}
              </Text>
              <TouchableOpacity style={styles.emptyBtn} onPress={() => router.push('/writing/create' as any)} activeOpacity={0.7}>
                <Text style={styles.emptyBtnText}>Create First Writing Test</Text>
              </TouchableOpacity>
            </View>
          }
        />
